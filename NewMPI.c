...
bool CalcularCombinacionOptima(int PrimeraCombinacion, int UltimaCombinacion, PtrListaArboles Optimo)//Calculate the optimal combination
{
    ...
    if ( myid == 0) {
		for ( islave =1; islave < numprocs ; islave ++) {
			MPI_Send ( &UltimaCombinacion, 1 , MPI_INTEGER , islave , MTAG1 , MPI_COMM_WORLD);
		}
	} else {
	MPI_Recv (&UltimaCombinacion, 1 , MPI_INTEGER , 0 , MTAG1 , MPI_COMM_WORLD ,&status);
	}
	...
	for (Combinacion=myid+PrimeraCombinacion; Combinacion<UltimaCombinacion; Combinacion+=numprocs)
	{
		...
		if ( Coste < CosteMejorCombinacion )
		{
			...
			MyData.Coste=CosteMejorCombinacion;
			MyData.Combinacion=MejorCombinacion;
	...
	if ( myid != 0) 
	{
		MPI_Send (&MyData, 1 , MPI_INTEGER,0 , MTAG2 , MPI_COMM_WORLD);
	} 
	else 
	{
		for ( islave =1; islave < numprocs ; islave ++) {
			MPI_Recv (&MyData, 1 ,MPI_INTEGER ,islave,MTAG2 , MPI_COMM_WORLD , &status );
			if ( MyData.Coste < CosteMejorCombinacion )
			{
				CosteMejorCombinacion = MyData.Coste;
				MejorCombinacion = MyData.Combinacion;
	      	  //   printf("***");
			}
		}
		run_time = MPI_Wtime() - start_time;
